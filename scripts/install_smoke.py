"""Run a clean installed-package smoke test for a built wheel."""
...
def _latest_wheel(dist_dir: Path) -> Path:
    ...
    wheels = sorted(
        dist_dir.glob("easyeda_monkey-*.whl"),
        key=lambda path: path.stat().st_mtime,
    )
    if not wheels:
        raise SystemExit(f"No easyeda_monkey wheel found in {dist_dir}")
    return wheels[-1]
...
def run_install_smoke(wheel: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="easyeda_monkey_install_smoke_") as temp:
        temp_dir = Path(temp)
        ...
        _run(
            [
                str(python),
                "-c",
                "import easyeda_monkey; print(easyeda_monkey.__version__)",
            ],
            cwd=temp_dir,
            env=env,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Wheel to install. Defaults to the newest easyeda_monkey wheel in dist/.",
    )
